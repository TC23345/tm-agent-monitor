...
- **Sidebar (left)** — the launch nav at the top (a folder switcher over one split *new session* row — the button starts whatever you started last, its chevron picks Claude Code / Codex / terminal — then Cursor / Chrome, the folder's own commands, New project, Projects folder; drop a folder from Explorer to launch there), then **Limits** and **Open windows**, each collapsing to its header (View → *Sidebar* toggles them).
- **Main frame** — up to **six panes**: the **Agents** pane (live sessions grouped by project, with nested subagents, waiting-only and collapse-all in its header), embedded **Terminal** panes (PowerShell, Claude Code, or Codex, in the launch folder), the **Activity** pane, and the **Usage** pane — today's spend, local usage insights, and thirty days of history with the model mix, in one scrolling column, opened from User → *Usage: spend & insights* (or View → Add pane, or the palette). Each pane header is an editor title bar: the kind, a folder chip, and that kind's tools — a terminal has split (another shell in the same folder), clear, restart, open external terminal here, and open folder; the launcher has new project and Projects folder — then zoom and close. Panes drag to reorder; the View menu picks the column count. Ctrl+Shift+` opens a new terminal pane, Ctrl+1…6 focuses a pane, Ctrl+Shift+←/→ steps between panes, Ctrl+, opens Settings. A terminal's `/` tool types common commands (`/compact`, `/cost`, `/clear`…) into it.
...
- **Limits** in the sidebar (bars only, Claude and Codex separately) so it stays scannable; token counts, spend, and local usage patterns together in the **Usage** pane.
