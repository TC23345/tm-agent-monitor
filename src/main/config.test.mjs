...
test('config source precedence and --mock are documented by the pure bootstrap contract', async (t) => {
  ...
  assert.equal(c.repoDir, join(root, 'Projects', 'claude-watch'))
  const packaged = bootstrapConfig({ isPackaged: true, userData, appData, cwd, home: root, env: { CLAUDE_WATCH_REPO: join(root, 'elsewhere') }, argv: [] })
  assert.equal(packaged.endpointFile, join(appData, 'taylormade-agent-monitor', 'hook-endpoint.json'))
  assert.equal(packaged.repoDir, join(root, 'elsewhere'))
})
