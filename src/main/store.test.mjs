...
test('settings patches allow only mutable fields with bounded runtime types', () => {
  ...
  assert.deepEqual(validateMutableSettingsPatch({ arrangeWindows: false }), { arrangeWindows: false })
  assert.equal(validateMutableSettingsPatch({ arrangeWindows: 'yes' }), null)
