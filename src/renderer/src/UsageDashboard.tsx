...
function ProviderLimits({ provider, accounts }: { provider: ProviderId; accounts: UsageAccount[] }) {
      ...
      {total === 0 ? (
        ...
        accounts.map((account) => (
          <Fragment key={account.id}>
            ...
            {account.week && <QuotaBar q={account.week} hint="Provider weekly usage window" />}
            {account.quotas?.map((quota) => (
              <QuotaBar key={`${quota.label}:${quota.resetsAt}`} q={quota} hint="Provider-scoped usage window" />
            ))}
          </Fragment>
        ))
      )}
...
export function UsageDashboard({ usage }: { usage: UsageSummary }) {
  return (
    <section className="usage">
      {byProvider(usage.accounts).map((group) => (
        <ProviderLimits key={group.provider} provider={group.provider} accounts={group.accounts} />
      ))}
    </section>
  )
}
