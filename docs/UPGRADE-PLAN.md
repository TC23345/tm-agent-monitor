...
1. **Idle cost + routing** — O1, F20, F13, F8, F2, F3. Renderer-only except the CDP chip. ✅ when shipped.
2. **Reports and launch** — F1, F5, F9, F17.
3. **Talk back** — F14, F18, F15, F16 (first IPC routes: project commands, git status).
4. **Feed and outside world** — F4, F19, F12.
5. **Deferred / larger** — F6 terminal tabs, F7 launch profiles, F10 multi-machine, F11 status API, O5, O6, O7 majors.
