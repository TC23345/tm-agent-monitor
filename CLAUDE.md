...
- `src/renderer/src/panes.ts` owns the layout contract: main-frame panes are instances (`{id, kind}`, `MAX_PANES = 6`) — `launcher` unique, `terminal` repeatable with per-pane launch config — plus the `SidebarView` catalog (limits, spend, windows, insights) toggled from the sidebar menu. The sidebar is an ordered stack of draggable slots (`SidebarSlot`, `tm.sidebarOrder.v1`) in which the agent list is itself a slot, so data views drag above or below it; a hidden view keeps its place, and each section rolls up to its header (Open windows starts collapsed). Views, order, collapse state, and panes all persist to localStorage. Uniqueness (and the sidebar toggle/collapse) is what keeps `windows:list` from being polled twice or while rolled up.
- `src/renderer/src/TopBar.tsx` is the app chrome, one IDE-style row: the logo glyph, the File / Terminal / View / User menus inline beside it, and the right edge holding passive status (waiting count, connection health) plus the window-control cluster — the half-view toggle (`window:half`, transient like Alt+Q, never writes `sizeMode`) and hide-to-tray. Every other app action lives in the menus; the sidebar and panes stay pure content — do not reintroduce a footer action strip. Shared dropdown pieces (gliding-highlight `MenuPop`, `MenuItem`, `MenuCheckItem`) live in `Menu.tsx`.
...
- The window covers the work area of the cursor's display (`positionWorkspace`) — full, or one half of it at full height per the persisted `sizeMode` setting ('full' | 'left' | 'right'), which is also the side the transient Alt+Q half view uses (left, unless the setting says right). Bounds never track content: there is deliberately no content-size IPC, no `setContentSize` call, and no renderer `ResizeObserver`. The `.app` card fills the window and each scroll region (`.pane-scroll`, `.gpane-body`) absorbs its own overflow; the grid's column count is capped in `colCap()` (App.tsx), not by media queries, so the View-menu column choice composes with the viewport. Do not reintroduce content-driven window sizing.
