...
- Interactive elements carry `data-testid` from `src/renderer/src/testid.ts` (`tid('menu', label)`, `pane-tool:<title>`, `pane:<kind>:<index>`, `agent:<id>`, `splitter-col-<i>`…). Automation targets those, never class names. `--remote-debugging-port` surfaces as `AppSettings.debugPort` and the `CDP :port` chip.
...
- Per-folder IPC: `project:commands` (parsed by `src/shared/projectCommands.mjs` from `.tm.json` + `package.json`, bounded) and `git:status` (`src/shared/gitStatus.mjs` parses `git status --porcelain=v1 -b`; main caches 30s, 4s timeout). Both validate that `cwd` is an existing directory and never throw to the renderer. `useProject.ts` polls them visibility-gated. `src/shared/digest.mjs` diffs the snapshot at hide against the one at summon for the away strip. A `TerminalPaneConfig.initialCommand` is typed once into a freshly created shell, never on reattach.
...
- `src/renderer/src/LaunchNav.tsx` is the head of the sidebar: a launch-target switcher (follow the active session, home, or any live project — or a folder dropped from Explorer, resolved by `path:describe`) over the launch rows, the folder's `.tm.json`/npm commands, and New project / Projects folder. Starting something is navigation, not work: there is no launcher pane, the grid holds only work, and `defaultPanes()` is empty (the grid shows an empty state instead of spawning a shell). A dropped `File` becomes a path through `webUtils.getPathForFile` in the **preload** — `File.path` was removed in Electron 32 and the sandboxed renderer never sees the object.
- `src/renderer/src/WorkspacePanes.tsx` holds the open-windows switcher. Terminal launches open embedded panes (Shift-click for an external window, and a full grid degrades to external). `useDesktopWindows` polls `windows:list` only while the document is visible, so a dismissed workspace stops enumerating.
...
- `src/renderer/src/usageShared.tsx` owns provider grouping for usage: fixed provider order, accounts sorted plan → local → API spend, and the shared quota bar. `UsageDashboard` renders limit bars only (the sidebar's Limits); `SpendView` owns token counts, per-project breakdowns, and budget/actual spend. `UsagePane` stacks `SpendView` then `UsageInsightsView` as the Usage pane — neither is reachable from the sidebar any more, and must not be: two mounted `UsageInsightsView`s would double the local session scans.
